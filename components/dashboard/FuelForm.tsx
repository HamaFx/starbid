...
export function FuelForm({ starId, claimToken }: { starId: string; claimToken: string }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <input name="amount" type="number" min="3" step="0.01" defaultValue="3" required className="w-full rounded-xl border border-white/10 bg-[#0a0a14] px-4 py-3 font-mono outline-none focus:border-[#4cc9f0]" />
