...
export function CheckoutFlow({ checkoutUrl, pendingBidId, rawToken }: CheckoutFlowProps) {
  ...
  const manageUrl = status?.star_id && rawToken ? buildManageUrl(window.location.origin, status.star_id, rawToken) : "";
    ...
    return (
      <section className="rounded-2xl border border-[#4ade80]/40 bg-[#0a0a14] p-6" aria-live="polite">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={() => { void navigator.clipboard?.writeText(manageUrl).then(() => setCopied(true)).catch(() => setCopied(false)); }} className="rounded-full border border-[#4cc9f0]/60 px-4 py-2 text-sm text-[#4cc9f0]">{copied ? "Copied" : "Copy manage link"}</button>
