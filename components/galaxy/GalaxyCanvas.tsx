...
export function GalaxyCanvas({ stars, filterTier = "all", searchQuery = "", onSelectStar, controllerRef }: {
  ...
  useEffect(() => { scene.updatePopulation(activeSorted.length); }, [activeSorted.length, scene.updatePopulation]);
  ...
  return (
    <div className="relative h-full w-full min-h-0 select-none overflow-hidden rounded-xl bg-[#07070b] cursor-grab active:cursor-grabbing">
      <div ref={hostRef} aria-label="Interactive celestial accretion disk — use mouse/trackpad to pan and zoom" role="application" className="h-full w-full min-h-0" />
      <div className="pointer-events-none absolute right-3 top-3 z-20 flex flex-col items-end gap-1 font-mono text-[9px] text-[#71717a]"><div className="flex items-center gap-1.5 rounded border border-white/10 bg-black/60 px-2 py-0.5 backdrop-blur-md"><span>CALIPER SCALE:</span><span className="font-bold text-[#38bdf8]">{(1 / scene.currentZoom).toFixed(2)} AU / div</span></div><div className="text-[8px] text-[#52525b]">[Double-click zoom · Scroll pan]</div></div>
