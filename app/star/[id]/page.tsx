...
export default async function StarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-[#05050a] px-6 py-16 text-[#fff4e0]"><div className="mx-auto max-w-2xl"><a href="/" className="font-mono text-sm text-[#4cc9f0]">← Back to galaxy</a>{star ? <LiveStarDetails star={star} stars={stars} /> : <><p className="mt-10 font-mono text-xs uppercase tracking-[0.2em] text-[#8f8c96]">Star {id}</p><h1 className="mt-3 text-4xl font-semibold">Project standing</h1><p className="mt-4 text-[#8f8c96]">This star is not available in the active public galaxy.</p></>}</div></main>;
}
