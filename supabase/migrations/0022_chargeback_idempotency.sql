...
begin
  ...
  join public.stars s on s.id = e.star_id
  join public.pending_bids b on b.id = e.pending_bid_id
  where b.lemonsqueezy_order_id = p_order_id
  limit 1;

  if v_project_id is null then
    raise exception 'order is not associated with a project';
  end if;

  -- Check for existing chargeback flag for idempotency
  ...
  insert into public.moderation_flags(project_id, reason, source)
  values (v_project_id, p_reason, 'admin')
